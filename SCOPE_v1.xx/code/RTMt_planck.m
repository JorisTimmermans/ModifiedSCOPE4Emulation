...
% Hcsui               = zeros(nl,1);      %             [nl]      modified by JT: not required anymore 
...
%% 1. calculation of upward and downward diffuse fluxes
...
for i = 1:length(IT);
    ...
    dummy           =   zeros(canopy.nlazi, canopy.nlayers);
    for j = 1:nl
        Hcsui2      =   Hcsui3(:,:,j);                 %  [nli,nlazi] sunlit  leaves        
        dummy(:,j)  =   Hcsui2'*lidf;        
%         Hcsui(j)    =   mean(Hcsui2'*lidf);             % Modified by JT: replaced by mean outside the iteration (faster( 
    end
    Hcsui           =   mean(dummy,1)';                 %  [nl]        sunlit  vegetation radiance per layer
    ...
    if obsdir
        ...
        for j = 1:nl        
            ...
            dummy(:,j)  = Hcsui2'*lidf;            
%             Hcsui(j)  = mean(Hcsui2'*lidf);         % Modified by JT: replaced by mean outside the iteration (is faster)
        end
        Hcsui           =   mean(dummy,1)';
