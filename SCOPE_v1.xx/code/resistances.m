...
% added by JT, modified so that two reference heights are possible

if isfield(resist_in,'zh') && isfield(resist_in,'zm')
    zref_m    =  resist_in.zm;
    zref_h    =  resist_in.zh;
    
elseif isfield(resist_in,'z')
    zref_m    =  resist_in.z;
    zref_h    =  resist_in.z;
end


% added by JT, input reference height is defined as relative to the vegetation height if zref<0
if zref_m<0;
    zref_m      =   h + abs(zref_m);
end
if zref_h<0
...
clear z
pm_z    	= psim(zref_m -d,L,unst,st);
ph_z    	= psih(zref_m -d,L,unst,st);
pm_h        = psim(h -d,L,unst,st);
%ph_h       = psih(h -d,L,unst,st);
ph_zr       = psih(zr-d,L,unst,st).*(zref_h>=zr) + ph_z.*(zref_h<zr);
phs_zr      = phstar(zr,zr,d,L,st,unst);
phs_h		= phstar(h ,zr,d,L,st,unst);

ustar   	= max(.001,kappa*u./(log((zref_m-d)/z0m) - pm_z));%          W&V Eq 30

Kh                  = kappa*ustar*(zr-d);                  %                W&V Eq 35
resist_out.Kh(unst)	= kappa*ustar(unst)*(zr-d).*(1-16*(h-d)./L(unst)).^0.5;% W&V Eq 35
...
rai = (zref_h>zr).*(1./(kappa*ustar).*(log((zref_h-d) /(zr-d))  - ph_z   + ph_zr));% W&V Eq 41 
