...
%
% Date: 	21 Sep 2012
% Update    26 Sep 2016 (JT: replaced a.^b, by exp(log(a)*b) for speed )
% Update:   20 Feb 2013
...
Kc          = Kcopt * exp(log(2.1).*qt);
Ko          = Koopt * exp(log(1.2).*qt);
kp          = kpopt.* exp(log(1.8).*qt);
Kd          = max(0.0301*(T-273.15)+ 0.0773,0.8738);
Rd          = Rdparam * Vcmo .* exp(log(1.8)*qt) ./(1+exp(1.3*(T-Trdm)));

switch Type
    case 'C3'
%         Vcmax       =           Vcmo .* 2.1.^qt ./TH * stressfactor;
        Vcmax       =           Vcmo .* exp(log(2.1)*qt) ./TH * stressfactor;
        
    otherwise 
%         Vcmax       =           Vcmo .* 2.1.^qt ./(TL.*TH) * stressfactor;
        Vcmax       =           Vcmo .* exp(log(2.1)*qt) ./(TL.*TH) * stressfactor;
end
...
x_alpha     = exp(alpha.*log(x));   % changed to increase speed
