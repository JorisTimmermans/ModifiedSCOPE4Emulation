...
% Extract MODTRAN atmosphere parameters at the SCOPE wavelengths
 t1  = atmo.M(:,1);                     % <Eso*costts/pi>
 t3  = atmo.M(:,2);                     % <r_atm_dd>
 t4  = atmo.M(:,3);                     % <t_atm_ss>
 t5  = atmo.M(:,4);                     % <t_atm_sd>
 t12 = atmo.M(:,5);                     % <t_atm_ssrdd>
 t16 = atmo.M(:,6);                     % <L_atm_a(b)>
...
rad.Pnh     = Pnhc;         % [60x1 double]     net PAR (moles m-2 s-1) of shaded leaves            % to be used in biochemical-module
rad.Pnu     = Pnuc;         % [13x36x60 double] net PAR (moles m-2 s-1) of sunlit leaves            % to be used in biochemical-module
